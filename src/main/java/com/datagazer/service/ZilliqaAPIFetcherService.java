...
public class ZilliqaAPIFetcherService {
    ...
    public List<BlockchainSummaryDto> getBlockchainSummaryList(){
       return jdbcTemplate.query("select max(coalesce(transaction_num,0)),avg(transaction_rate) as transactionRate,avg(tx_block_num) as txBlockNum,avg(zil_price) as zilPrice, day_added as dayAdded from blockchain_summary group by day_added order by day_added desc limit 7",new BeanPropertyRowMapper(BlockchainSummaryDto.class));
    }
